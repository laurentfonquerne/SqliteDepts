...
private class DbInit extends SQLiteOpenHelper {
    ...
    public DbInit(Context context, String name, int version) {
        super(context, name, null, version); //appeler le constructeur du parent
        ctxt = context;
    }
    ...
    public static DbInit getInstance(Context ctxt){
        if (instance == null){
            instance = new DbInit(ctxt, "geo",1);
        }
        return instance;
    }
// la méthode OnUpgrade sera exécutée si on change de version de la base...

    @Override
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        //affNotif(ctxt, "MAJ de la BDD");
    }
